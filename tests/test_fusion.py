...
from soap.semantics.label import LabelContext
...
class TestFusion(unittest.TestCase):
    ...
    def check(self, case):
        ...
        label_state = LabelGenerator(LabelContext(state)).execute(
            state, BoxState(bottom=True))[1]
        new_state = fusion(label_state, flow.outputs)
