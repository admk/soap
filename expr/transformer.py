...
def _walk_r(t, f, v, c):
    ...
    for e in f(t):
        s.add(e)
    for e in _walk_r(t.arg1, f, v, c):
        s.add(Expr(op=t.op, arg1=e, arg2=t.arg2))
    for e in _walk_r(t.arg2, f, v, c):
        s.add(Expr(op=t.op, arg1=t.arg1, arg2=e))
...
class ExprTreeTransformer(TreeTransformer):
    ...
    def associativity(self, t):
        ...
        if is_expr(t.arg1):
            if t.arg1.op == t.op:
                s.append(Expr(op=t.op,
                              arg1=t.arg1.arg1,
                              arg2=Expr(op=t.op, arg1=t.arg1.arg2, arg2=t.arg2)))
        if is_expr(t.arg2):
            if t.arg2.op == t.op:
                s.append(Expr(op=t.op,
                              arg1=Expr(op=t.op, arg1=t.arg1, arg2=t.arg2.arg1),
                              arg2=t.arg2.arg2))
        return s
    ...
    def distribute_for_distributivity(self, t):
        s = []
        if t.op in self.LEFT_DISTRIBUTIVITY_OPERATORS and is_expr(t.arg2):
            if (t.op, t.arg2.op) in self.LEFT_DISTRIBUTIVITY_OPERATOR_PAIRS:
                s.append(
                    Expr(op=t.arg2.op,
                         arg1=Expr(op=t.op, arg1=t.arg1, arg2=t.arg2.arg1),
                         arg2=Expr(op=t.op, arg1=t.arg1, arg2=t.arg2.arg2)))
        if t.op in self.RIGHT_DISTRIBUTIVITY_OPERATORS and is_expr(t.arg1):
            if (t.op, t.arg1.op) in self.LEFT_DISTRIBUTIVITY_OPERATOR_PAIRS:
                s.append(
                    Expr(op=t.arg1.op,
                         arg1=Expr(op=t.op, arg1=t.arg1.arg1, arg2=t.arg2),
                         arg2=Expr(op=t.op, arg1=t.arg1.arg2, arg2=t.arg2)))
        return s

    def collect_for_distributivity(self, t):
        op, arg1, arg2 = t.tuple()
        if not op in (self.LEFT_DISTRIBUTION_OVER_OPERATORS +
                    self.RIGHT_DISTRIBUTION_OVER_OPERATORS):
            return []
        # depth test
        if not is_expr(arg1) and not is_expr(arg2):
            return []
        # expand by adding identities
        if is_expr(arg2):
            op2, arg21, arg22 = arg2.tuple()
            if op2 == MULTIPLY_OP:
                if arg21 == arg1:
                    arg1 = Expr(op=op2, arg1=arg1, arg2=1L)
                elif arg22 == arg1:
                    arg1 = Expr(op=op2, arg1=1L, arg2=arg1)
        if is_expr(arg1):
            op1, arg11, arg12 = arg1.tuple()
            if op1 == MULTIPLY_OP:
                if arg11 == arg2:
                    arg2 = Expr(op=op1, arg1=arg2, arg2=1L)
                elif arg12 == arg2:
                    arg1 = Expr(op=op1, arg1=1L, arg2=arg2)
        # must be all expressions
        if not is_expr(arg1) or not is_expr(arg2):
            return []
        # equivalences
        op1, arg11, arg12 = arg1.tuple()
        op2, arg21, arg22 = arg2.tuple()
        ...
        if (op1, op) in self.LEFT_DISTRIBUTIVITY_OPERATOR_PAIRS:
            if arg11 == arg21:
                s.append(Expr(op=op1,
                              arg1=arg11,
                              arg2=Expr(op=op, arg1=arg12, arg2=arg22)))
        if (op1, op) in self.RIGHT_DISTRIBUTIVITY_OPERATOR_PAIRS and \
                (op2, op) in self.RIGHT_DISTRIBUTIVITY_OPERATOR_PAIRS:
            if arg12 == arg22:
                s.append(Expr(op=op2,
                              arg1=Expr(op=op, arg1=arg11, arg2=arg21),
                              arg2=arg12))
        return s

    def commutativity(self, t):
        return [Expr(op=t.op, arg1=t.arg2, arg2=t.arg1)]
    ...
    def zero_reduction(self, t):
        if t.op != MULTIPLY_OP:
            return t
        if t.arg1 != 0 and t.arg2 != 0:
            return t
        return 0

    def constant_reduction(self, t):
        if not is_num(t.arg1) or not is_num(t.arg2):
            return t
        if t.op == MULTIPLY_OP:
